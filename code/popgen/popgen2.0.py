# "Pop Music Generator"
# Bart Massey 2024
# Adjusted by Avesta Mirashrafi by adding an envelope, an alternate
# harmonic sequence, an alternate 3/4 time signature, and a clamped 
# melodic line.
#
...
if random.random() > 0.5:
    chord_loop = [8, 5, 6, 4]
else: #alternate chord progression
    chord_loop = [8, 2, 6, 5]

time = 4 #default 4/4
if random.random() > 0.5: #chance for 3/4 time
    time = 3


print(time)
position = 0
def pick_notes(chord_root, time): #added time instead of hardcoded 4
    global position
    p = position
    ...
    for _ in range(time):
        ...
        #clamped melodic line in range -20 and 20
        melody_note = min(max(chord_note, -20), 20)
        notes.append(melody_note)


        print(f"Chord Note: {chord_note}") #Used for testing
...
def make_note(key, n=1):
    ...
    #create envelope
    envelope = np.ones(len(sine_wave))
    fade_length = int(0.01 * samplerate)  #portion of sample rate used for fade

    #create line space for fade in and out
    ...
    return sine_wave * envelope #apply envelope to wave
